...
export default function Navbar() {
  ...
  return (
    <>
      ...
      {/* Mobile menu */}
      <div 
        className={`lg:hidden fixed inset-0 top-[72px] sm:top-[88px] z-40 bg-primary-bg/98 backdrop-blur-xl transition-all duration-500 transform ${
          open ? "translate-x-0 opacity-100" : "translate-x-full opacity-0"
        } ${scrolled ? "!top-[64px] sm:!top-[72px]" : ""}`}
      >
        <div className="p-8 space-y-4 h-full overflow-y-auto [scrollbar-width:none] [-ms-overflow-style:none] [&::-webkit-scrollbar]:display-none">
          {NAV.map((n) => {
            const active = path === n.to;
            return (
              <Link
